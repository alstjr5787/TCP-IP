...
#include <arpa/inet.h> // 192.168.0.7 -> network 값으로 바꿔주는 라이브러리
#include <pthread.h>
#include <semaphore.h> // mutex
#include "SerialPort.cpp" // Serial 통신을 위한 라이브러리

void* task(void*); // thread 함수 => task main
void* taskSerial(void*); // thread 함수 => 전송용 thread
static pthread_mutex_t mutex; // key 하나씩 처리하기 위해
static std::vector<int> client_socket_fds; // 클라이언트들의 fd 저장하는 자료구조
static int serial_port = 0; // 시리얼포트 저장용 변수

// 네트워크상에서 문자열 저장 array
static std::array<char, BUFSIZ> tcp_message; //buffer
static std::array<char, BUFSIZ> serial_message; // buffer -> 인계형
...
int main(int argc, const char* argv[])
{
    ...
    pthread_t pid; // thread를 담기위한 변수
    if(argc != 2)
    {
        std::cout << "./SERVER 9999" << std::endl;
        exit(EXIT_FAILURE); // exit(1);
    }
    serialport_init("/dev/ttyACM0", 115200, nullptr);
    if(serial_port == -1)
    {
    ...
    server_addr.sin_port = htons(atoi(argv[1])); // port 번호가 들어감
    ...
    while(true)
    {
        ...
        pthread_mutex_unlock(&mutex); // 락풀기
